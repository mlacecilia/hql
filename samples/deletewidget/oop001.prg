...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlLabel( "theLabel" )
         ...
         :setText( "to be removed" )
      END WITH

      WITH OBJECT hqlPushButton()
         :hqlAddMeToLayout( oMlayout )
         :setText( "press to test" )
         :hqlOnClicked( { || UDFdeleteWidget( oWnd ) } )
      END WITH
...
STATIC PROCEDURE UDFdeleteWidget( oWnd )
   LOCAL lFound

   hql_exploreChildren( oWnd, .T. )

   lFound := UDFdummy( oWnd )

   IF ( lFound )
      hql_Trace( "found: remove" )
      hql_exploreChildren( oWnd, .T. )
   ELSE
      hql_Trace( "NOT found" )
   ENDIF
...
/*
   dummy function used to not have delay using tracing and go out of scope oWdg var
*/
STATIC FUNCTION UDFdummy( oWnd )
   LOCAL oWdg := oWnd:findChild( "THELABEL", Qt_FindDirectChildrenOnly )
   LOCAL lFound

   IF ( hb_IsObject( oWdg ) )
      lFound := .T.
      hql_DeleteWidget( oWdg )
      oWdg := NIL
   ELSE
      lFound := .F.
   ENDIF
