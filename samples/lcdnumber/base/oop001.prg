...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLLCDNUMBER tester" )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlLcdNumber("mylcd")
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :setMinimumSize( 400, 200 )
      END WITH

      WITH OBJECT hqlSlider(/*name*/)
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :setRange( 0, 100 )
         :setOrientation( Qt_Horizontal )
         :setTickInterval( 10 )
         :setTickPosition( QSlider_TicksAbove )
         :hqlOnValueChanged( { |nint| oWnd:mylcd:display(nint) } )
      END WITH
   END WITH
