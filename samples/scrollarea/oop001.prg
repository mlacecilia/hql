...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLSCROLLAREA tester" )
      :setWindowIcon( QIcon( ":/hqlres/HQL96" ) )
      :setCentralWidget( hqlScrollArea(/*name*/) )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlWidget(/*name*/)
         :resize( 6000, 3000 )
         :hqlSetMeAsWidgetOf( oWnd:centralWidget() ) //IOW oWnd:centralWidget():setWidget

         WITH OBJECT hqlPushButton()
            :setGeometry( 10, 10, 250, 40 )
            :setText( "#1 button" )
            :hqlOnClicked( { || UDFtest() } )
         END WITH

         WITH OBJECT hqlPushButton()
            :setGeometry( 10, 400, 250, 40 )
            :setText( "#2 button" )
            :hqlOnClicked( { || UDFtest() } )
         END WITH

         WITH OBJECT hqlPushButton()
            :setGeometry( 10, 900, 250, 40 )
            :setText( "#3 button" )
            :hqlOnClicked( { || UDFtest() } )
         END WITH

         WITH OBJECT hqlPushButton()
            :setGeometry( 10, 2000, 250, 40 )
            :setText( "last button" )
            :hqlOnClicked( { || UDFtest() } )
         END WITH
