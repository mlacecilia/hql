...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLCOMMANDLINKLBUTTON tester" )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlCommandLinkButton("bt1")
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :hqlCaption( "press/clicked" )
         :hqlOnClicked( { |oSelf| UDFclicked( oSelf ) } )
         :hqlOnPressed( { |oself| UDFpressed( oSelf ) } )
      END WITH

      WITH OBJECT hqlCommandLinkButton("bt2")
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :hqlCaption( "press/clicked" )
         :hqlOnClicked( { |oSelf| UDFclicked( oSelf ) } )
...
STATIC PROCEDURE UDFpressed( oSelf )
