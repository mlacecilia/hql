...
METHOD __cleaner() CLASS oop002
   ::oWnd := NIL
RETURN NIL

METHOD __createUi() CLASS oop002
   LOCAL oSize, oMlayout

   WITH OBJECT ::oWnd := hqlMainWindow("mainwin")
      :setWindowTitle( "HQL*window* tester" )
      :setCentralWidget( hqlWidget(/*name*/) )
      :centralWidget():setLayout( QVBoxLayout() )
      :hqlOnClose( { || ::__cleaner() } )
   END WITH

   oMlayout :=  ::oWnd:centralWidget():layout()
...
METHOD __setBackGround() CLASS oop002
   ::oWnd:setStyleSheet( "QMainWindow#MAINWIN {background-image: url(:pgmhome);background-repeat:no-repeat;background-position:center;}")
RETURN NIL

METHOD __call_oop003() CLASS oop002
   LOCAL oPgm := oop003():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop004() CLASS oop002
   LOCAL oPgm := oop004():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop005( oparent ) CLASS oop002
   LOCAL oPgm := oop005():new( oparent )
   oPgm:activate()
RETURN NIL

METHOD __call_oop006( oparent ) CLASS oop002
