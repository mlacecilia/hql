...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      ...
      WITH OBJECT hqlMenuBar(/*name*/)
         ...
         WITH OBJECT :hqlAddMenu(/*name*/)
            ...
            WITH OBJECT :hqlAddAction(/*name*/)
               :hqlCaption( "&Clear" )
               :hqlOnTriggered( { || oWnd:mydial:clear() } )
            END WITH
            WITH OBJECT :hqlAddMenu(/*name*/)
               :hqlCaption( "cu&Rrent" )
               WITH OBJECT :hqlAddAction(/*name*/)
                  :hqlCaption( "&Get" )
                  :hqlOnTriggered( { || hql_MsgStop( "value is: " + hb_NtoS(oWnd:mydial:hqlValue()) ) } )
               END WITH
               WITH OBJECT :hqlAddAction(/*name*/)
                  :hqlCaption( "&Set (50)" )
                  :hqlOnTriggered( { || oWnd:mydial:hqlValue(50) } )
               END WITH
            END WITH
         END WITH
   ...
   WITH OBJECT oWnd:centralWidget()
      oMlayout := :layout()
      WITH OBJECT hqlDial( "mydial" )
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :setRange( 0, 100 )
         :setNotchesVisible( .T. )
         :hqlOnValueChanged( { |nint, oself| UDFindexChanged(nint,oself) } )
      END WITH
   END WITH
