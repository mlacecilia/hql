...
METHOD __cleaner() CLASS oop006
   ::oWnd := NIL
RETURN NIL

METHOD __createUi( oParent ) CLASS oop006
   LOCAL oSize, oMlayout

   WITH OBJECT ::oWnd := hqlModalDialog(/*name*/, oParent)
      :setWindowTitle( "HQLMODALDIALOG #"+hb_NtoS(++::s_nCount) )
...
METHOD __call_oop003() CLASS oop006
   LOCAL oPgm := oop003():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop004() CLASS oop006
   LOCAL oPgm := oop004():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop005( oparent ) CLASS oop006
   LOCAL oPgm := oop005():new( oparent )
   oPgm:activate()
RETURN NIL

METHOD __call_oop006( oparent ) CLASS oop006
