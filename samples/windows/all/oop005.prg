...
METHOD __cleaner() CLASS oop005
   ::oWnd := NIL
RETURN NIL

METHOD __createUi( oParent ) CLASS oop005
   LOCAL oSize, oMlayout

   WITH OBJECT ::oWnd := hqlModalWindow(/*name*/, oParent)
      :setWindowTitle( "HQLMODALWINDOW #"+hb_NtoS(++::s_nCount) )
   ...
   ELSE
      ...
      oSize := QSize( oSize:width()*0.8, oSize:height()*0.8 )
   ENDIF
   ::oWnd:resize( oSize )

RETURN NIL

METHOD __call_oop003() CLASS oop005
   LOCAL oPgm := oop003():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop004() CLASS oop005
   LOCAL oPgm := oop004():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop005( oparent ) CLASS oop005
   LOCAL oPgm := oop005():new( oparent )
   oPgm:activate()
RETURN NIL

METHOD __call_oop006( oparent ) CLASS oop005
