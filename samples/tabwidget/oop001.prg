...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLTABWIDGET tester" )
      ...
      WITH OBJECT hqlMenuBar(/*name*/)
         ...
         WITH OBJECT :hqlAddMenu( /*name*/ )
            ...
            WITH OBJECT :hqlAddAction(/*name*/)
               :hqlCaption( "&Add" )
               :hqlOnTriggered( { || UDFaddTab(oWnd:mytable()) } )
            END WITH
            WITH OBJECT :hqlAddAction(/*name*/)
               :hqlCaption( "&Insert" )
               :hqlOnTriggered( { || UDFinsertTab(oWnd:mytable()) } )
            END WITH
            WITH OBJECT :hqlAddAction(/*name*/)
               :hqlCaption( "&Remove" )
               :hqlOnTriggered( { || oWnd:mytable:removeTab(0) } )
            END WITH
         END WITH
   ...
   WITH OBJECT oWnd:centralWidget()
      oMlayout := :layout()
      WITH OBJECT hqlTabWidget("mytable")
         :hqlAddMeToLayout( oMlayout )
         /* :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter ) */
         :hqlOnCurrentChanged( { |nint,oself| UDFonCurrentChanged(nint,oSelf) } )
      END WITH
   END WITH
...
STATIC FUNCTION UDFcounter()
   STATIC nCount := 0
   nCount += 1
RETURN nCount

STATIC PROCEDURE UDFaddTab( otable )
//   LOCAL cName := "tab_"
   LOCAL cLabel := "label_"
   LOCAL nCount := UDFcounter()

//   cName += hb_NtoS(nCount)
   cLabel += hb_NtoS(nCount)

   oTable:hqlAddTab( /*cName*/, cLabel, QIcon(":/hqlres/dummy") )
RETURN

STATIC PROCEDURE UDFinsertTab( otable )
//   LOCAL cName := "tab_"
   LOCAL cLabel := "label_"
   LOCAL nCount := UDFcounter()
...
//   cName += hb_NtoS(nCount)
   ...
   oTable:hqlInsertTab( /*cName*/, cLabel, QIcon(":/hqlres/dumy"), 0 )
RETURN

STATIC PROCEDURE UDFonCurrentChanged(nint,oSelf)
   hql_Trace( PADR("onCurrentChanged: ",25) + "nint: " + hb_NtoS(nint) + " " + oself:objectName() + " " + hb_TtoS(hb_DateTime()) )
RETURN
