...
*/
CREATE CLASS oop003 INHERIT basic_form
...
*/
METHOD __createWin() CLASS oop003
   ...
   WITH OBJECT ::oWindow := hqlChildWindow( /*name*/, ::parentWindow() )
      ...
      WITH OBJECT :centralWidget()
         ...
         WITH OBJECT :hqlAddColumn()
            ...
            END WITH
            WITH OBJECT :cell()
               :setTextAlignment( hb_BitOr( Qt_AlignRight, Qt_AlignVCenter ) )
            END WITH
         END WITH
...
*/
METHOD __loadForm() CLASS oop003

   LOCAL oRow
   LOCAL cNumber

   (::oCounters:wa)->(ORDSETFOCUS("bycode"))
   (::oCounters:wa)->(DBGOTOP())

   DO WHILE ! (::oCounters:wa)->(EOF())

      oRow := ::oWindow:grid:hqlAddRow()
      oRow[ 1 ]:setText( (::oCounters:wa)->(FIELDGET(1)) )
      cNumber := hql_Transform( (::oCounters:wa)->(FIELDGET(3)), "99,999,999" )
      oRow[ 2 ]:setText( cNumber )
      oRow[ 3 ]:setText( (::oCounters:wa)->(FIELDGET(2)) )
...
*/
METHOD __recAdd() CLASS oop003

   LOCAL oProgram

   oProgram := oop004():new( Self )
...
*/
METHOD __recSelect() CLASS oop003
