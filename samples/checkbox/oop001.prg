...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLCHECKBOX tester" )
         ...
         WITH OBJECT :hqlAddMenu( /*name*/ )
            ...
            WITH OBJECT :hqlAddMenu( /*name*/ )
               ...
               WITH OBJECT :hqlAddAction( /*name*/ )
                  ...
                  :hqlOnTriggered( { || hql_MsgStop( "current state is="+hb_NtoS(oWnd:onoff:hqlValue()) ) } )
               END WITH
               WITH OBJECT :hqlAddAction( /*name*/ )
                  :hqlCaption( "&Set 2" )
                  :hqlOnTriggered( { || oWnd:onoff:hqlValue(2) } )
               END WITH
            END WITH
            :addSeparator()
            WITH OBJECT :hqlAddMenu( /*name*/ )
               ...
               WITH OBJECT :hqlAddAction( /*name*/ )
                  :hqlCaption( "&Current is" )
                  :hqlOnTriggered( { || hql_MsgStop( "current state is="+hb_NtoS(oWnd:tristate:hqlValue()) ) } )
               END WITH
               WITH OBJECT :hqlAddAction( /*name*/ )
                  :hqlCaption( "&Set 1" )
                  :hqlOnTriggered( { || oWnd:tristate:hqlValue(1) } )
               END WITH
            END WITH
         END WITH
   ...
   WITH OBJECT oWnd:centralWidget()
      oMlayout := :layout()
      WITH OBJECT hqlCheckBox( "onoff" )
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :setText( "On Off" )
         :hqlOnStateChanged( { |nState, oSelf| UDFstateChanged( nState, oSelf ) } )
      END WITH
      WITH OBJECT hqlCheckBox( "tristate" )
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :setText( "Tristate" )
         :hqlOnStateChanged( { |nState, oSelf| UDFstateChanged( nState, oSelf ) } )
...
STATIC PROCEDURE UDFstateChanged( nState, oSelf )

   hql_Trace( PADR("stateChanged: ",25) + ;
                      "state: " + hb_NtoS(nState) + " " + ;
