...
PROCEDURE Main()
   ...
   oFontLib := hqlUbuntuFont()
   lRegistered := hqlRegisterResData( oFontLib:rccData() )
   hql_Trace( "registered: " + hb_ValToExp(lRegistered) )

   // add font to application
   FOR EACH cFile IN oFontLib:fontList()
      nFontId := HqlAddApplicationFont( cFile )
      hql_Trace( cFile + " fontId: " + hb_NtoS(nFontId) )
   NEXT

   // change application font
   oFont := QFont( "Ubuntu", -1, QFont_Normal )
...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      this := :hqlThis()
      :setWindowTitle( "EXTERNAL lib. FONTS tester" )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      oFont := QFont( "Ubuntu", -1, QFont_Normal, .T. )
