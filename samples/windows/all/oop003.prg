...
METHOD __cleaner() CLASS oop003
   ::oWnd := NIL
RETURN NIL

METHOD __createUi( oParent ) CLASS oop003
   LOCAL oSize, oMlayout

   WITH OBJECT ::oWnd := hqlChildWindow(/*name*/, oParent)
      :setWindowTitle( "HQLCHILDWINDOW #"+hb_NtoS(++::s_nCount) )
...
METHOD __call_oop003() CLASS oop003
   LOCAL oPgm := oop003():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop004() CLASS oop003
   LOCAL oPgm := oop004():new( ::oWnd )
   oPgm:activate()
RETURN NIL

METHOD __call_oop005( oparent ) CLASS oop003
   LOCAL oPgm := oop005():new( oparent )
   oPgm:activate()
RETURN NIL

METHOD __call_oop006( oparent ) CLASS oop003
