...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "recents HQLMENU tester" )
      ...
      WITH OBJECT hqlMenuBar( "mmenu" )
         ...
         WITH OBJECT :hqlAddMenu("mnurecents")
            :hqlCaption( "&Recents" )
            :hqlRecentEnabled( .T. )   // mandatory
            :hqlSetMaxRecents( /*numeric = 10 */ )
            :hqlOnRecentSelected( { |lbool,oaction| UDFonRecentSelected(lbool,oaction) } )
         END WITH
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlPushButton(/*name*/)
         ...
         :hqlCaption( "Clear recent list" )
         :hqlOnClicked( { || oWnd:mnurecents:hqlClearRecent() } )
      END WITH

      WITH OBJECT hqlPushButton(/*name*/)
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :hqlCaption( "Load recent list" )
         :hqlOnClicked( { || UDFloadrecents(oWnd) } )
      END WITH
...
STATIC PROCEDURE UDFloadrecents(oWnd)
   LOCAL aList := {}
   LOCAL cPath := hb_DirBase()

   AADD( aList, hb_FnameMerge(cPath, "file_1", "txt") )
   AADD( aList, hb_FnameMerge(cPath, "file_2", "txt") )
   AADD( aList, hb_FnameMerge(cPath, "file_3", "txt") )
   AADD( aList, hb_FnameMerge(cPath, "file_4", "txt") )

   oWnd:mnurecents:hqlSetRecentList( aList )
   // OR
   oWnd:mnurecents:hqlAddToRecent( hb_FnameMerge(cPath, "file_5", "txt") )

RETURN

STATIC PROCEDURE UDFonRecentSelected(lbool,oaction)
   hql_Trace( "bool: " + hb_ValToExp(lbool) + " text: " + oaction:text() )
RETURN
