...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLTEXTBROWSER tester" )
      :setWindowIcon( QIcon( ":/hqlres/HQL96" ) )
      :setCentralWidget( hqlTextBrowser(/*name*/) )
      ...
      WITH OBJECT hqlMenuBar(/*name*/)
         ...
         WITH OBJECT :hqlAddMenu( /*name*/ )
            :hqlCaption( "&Tools" )
            WITH OBJECT :hqlAddAction(/*name*/)
               :hqlCaption( "show page &1" )
               :hqlOnTriggered( { || oWnd:centralWidget:setSource( QUrl( "pagina.html" ) ) } )
            END WITH
            WITH OBJECT :hqlAddAction(/*name*/)
               :hqlCaption( "show page &2" )
               :hqlOnTriggered( { || oWnd:centralWidget:setSource( QUrl( "pagina2.html" ) ) } )
            END WITH
         END WITH
   ...
   WITH OBJECT oWnd:centralWidget()
      :hqlOnSourceChanged( { |ourl| hql_Trace( ourl:toString() ) } )
      :hqlOnBackwardAvailable( { |lbool| hql_Trace( "BackwardAvailable=" + hb_ValToExp( lbool ) ) } )
      :hqlOnForwardAvailable( { |lbool| hql_Trace( "ForwardAvailable=" + hb_ValToExp( lbool ) ) } )
   END WITH
