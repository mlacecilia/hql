...
PROCEDURE Main()
   ...
   HqlHelp:searchPaths:append( hb_Cwd() )

   hqlStart()

   UDFshowMainWindow()

   HqlHelp:quit() // mandatory
...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlPushButton()
         ...
         :setText( "show path list" )
         :hqlOnClicked( { || UDFshowPathList() } )
      END WITH

      WITH OBJECT hqlLineEdit()
         :hqlAddMeToLayout( oMlayout )
         :setPlaceHolderText( "<SHIFT+F1> to show menu" )
         :hqlSetFKey( Qt_Key_F1, Qt_ShiftModifier, {|oself| HqlHelp:showHelp( "pagina2.html#cap3", oself ) } )
         END WITH
...
STATIC PROCEDURE UDFshowPathList()
   LOCAL oSlist := HqlHelp:searchPaths()
   LOCAL nAt

   FOR nAt := 0 TO (oSlist:size()-1)
      hql_Trace( oSlist:at(nAt) )
   NEXT
