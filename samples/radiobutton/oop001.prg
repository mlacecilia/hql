...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   LOCAL oFont := hqlQApplication:font()

   oFont:setPointSize( 12 )

   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLRADIOBUTTON tester" )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlGroupBox(/*name*/)
         ...
         :hqlCaption( "auto exclusive" )
         :setLayout( hqlVBoxLayout(/*name*/) )
         :setFont( oFont )
         oLayout := :layout()

         WITH OBJECT hqlRadioButton( "excl_1" )
            :hqlAddMeToLayout( oLayout )
            :hqlAlignMeToLayout( oLayout, Qt_AlignCenter )
            :setText( "Exclusive_1" )
            :hqlOnToggled( { |lBool, oSelf| UDFtoggled( lBool, oSelf ) } )
         END WITH
         WITH OBJECT hqlRadioButton( "excl_2" )
            :hqlAddMeToLayout( oLayout )
            :hqlAlignMeToLayout( oLayout, Qt_AlignCenter )
            :setText( "Exclusive_2" )
            :hqlOnToggled( { |lBool, oSelf| UDFtoggled( lBool, oSelf ) } )
      ...
      WITH OBJECT hqlGroupBox(/*name*/)
         :hqlAddMeToLayout( oMlayout )
         :hqlCaption( "NO auto exclusive" )
         :setLayout( hqlVBoxLayout(/*name*/) )
         :setFont( oFont )
         oLayout := :layout()

         WITH OBJECT hqlRadioButton( "noexcl_1" )
            :hqlAddMeToLayout( oLayout )
            :hqlAlignMeToLayout( oLayout, Qt_AlignCenter )
            :setText( "NotExclusive_1" )
            :setAutoExclusive( .F. )
            :hqlOnToggled( { |lBool, oSelf| UDFtoggled( lBool, oSelf ) } )
         END WITH
         WITH OBJECT hqlRadioButton( "noexcl_2" )
            :hqlAddMeToLayout( oLayout )
            :hqlAlignMeToLayout( oLayout, Qt_AlignCenter )
            :setText( "NotExclusive_2" )
            :setAutoExclusive( .F. )
            :hqlOnToggled( { |lBool, oSelf| UDFtoggled( lBool, oSelf ) } )
         END WITH
      END WITH
...
STATIC PROCEDURE UDFtoggled( lBool, oSelf )

   hql_Trace( PADR("stateChanged: ",25) + ;
                      "state: " + hb_ValToExp(lBool) + " " + ;
