...
/*!
   ...
   ALERT! about HQL_VER_STATUS always lowercase; eg. "alpha", "beta", "stable"
...
/*!
 ...
 \brief returns version number following https://semver.org/
 \param(IN)
 \return string

*/
FUNCTION hql_Version()
   LOCAL cVersion := hb_NtoS( HQL_VER_MAJOR ) + "." + hb_NtoS( HQL_VER_MINOR ) + "." + hb_NtoS( HQL_VER_PATCH )
   IF( !hb_IsNull( HQL_VER_STATUS ) )
      cVersion += "-" + HQL_VER_STATUS
   ENDIF
   IF( HQL_VER_BUILD > 0 )
      cVersion += "+" + hb_NtoS( HQL_VER_BUILD )
   ENDIF
RETURN cVersion

/*!

 \brief returns "HQL " + version number
 \param(IN)
 \return string

*/
FUNCTION hql_VerNamed()
RETURN ( "HQL " + hql_Version() )
