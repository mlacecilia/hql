...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      this := :hqlThis()
      :setWindowTitle( "HQLDOCKWIDGET tester" )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlLabel(/*name*/)
         ...
         :hqlCaption( "cental label" )
         :setMinimumSize( QSize( 300, 300 ) )
      END WITH
   END WITH

   WITH OBJECT oWnd
      WITH OBJECT oDock := hqlDockWidget( "dock1" )
         :resize( 200, 250 )
         :hqlOnDockLocationChanged( { |n,oself| UDFlocacationChanged( n, oself ) } )
         WITH OBJECT hqlLabel(/*name*/)
            :setText( "labelDock_1" )
            :hqlSetMeAsWidgetOf( oDock )
         END WITH
      END WITH
      :addDockWidget( Qt_RightDockWidgetArea, oDock )

      WITH OBJECT oDock := hqlDockWidget( "dock2" )
         :resize( 200, 250 )
         :hqlOnDockLocationChanged( { |n,oself| UDFlocacationChanged( n, oself ) } )
         WITH OBJECT hqlLabel( /*name*/ )
            :setText( "labelDock_2" )
...
STATIC PROCEDURE UDFlocacationChanged( n, oself )

   hql_Trace( "dockwidget name=" + oSelf:objectName() + " newArea= " + hb_NtoC( n ) )
