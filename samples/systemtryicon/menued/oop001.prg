...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLSYSTEMTRAYICON tester" )
      ...
      WITH OBJECT hqlSystemTrayIcon(/*cName*/)
         :setIcon( QIcon( ":/hqlres/HQL96" ) )
         :setToolTip( "HQL SystemTrayIcon tooltip" )
         :hqlOnMessageClicked( { || hql_Trace( "clicked on baloon-message" ) } )
         :hqlOnActivated( { | n | hql_Trace( "reason is " + hb_NtoS(n) ) } )
         WITH OBJECT :hqlAddMenu( /*cName*/ )
            WITH OBJECT :hqlAddAction( /*name*/ )
               :setText( "&Quit" )
               :setIcon( QIcon( ":/hqlres/quit" ) )
               :setShortcut( QKeySequence( "Alt+Q" ) )
         ...
         END WITH
         :show()
         :showMessage( "HQL SystemTrayIcon", "HQL works", QSystemTrayIcon_Information, 5000 )
      END WITH
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlLabel()
         ...
         :hqlFontBold( .T. )
         :hqlCaption( "You can 'L-click' on baloon-message" )
      END WITH
      ...
      WITH OBJECT hqlLabel()
         ...
         :hqlAlignMeToLayout( oMlayout, Qt_AlignHCenter )
         :hqlFontBold( .T. )
         :hqlCaption( "You can 'L-click' or 'R-click' or... on icon to know reason" )
      END WITH

      WITH OBJECT hqlLabel()
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignHCenter )
         :hqlCaption( "empty text" )
      END WITH
