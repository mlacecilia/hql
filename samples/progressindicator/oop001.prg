...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLPROGRESSINDICATOR tester" )
   ...
   WITH OBJECT oWnd:centralWidget()
      ...
      WITH OBJECT hqlPushButton(/*name*/)
         ...
         :hqlCaption( "Start" )
         :hqlOnClicked( { || oWnd:myprogress:startAnimation() } )
      END WITH
      ...
      WITH OBJECT hqlPushButton(/*name*/)
         ...
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :hqlCaption( "Stop" )
         :hqlOnClicked( { || oWnd:myprogress:stopAnimation() } )
      END WITH

      WITH OBJECT hqlProgressIndicator("myprogress")
         :hqlAddMeToLayout( oMlayout )
         :hqlAlignMeToLayout( oMlayout, Qt_AlignCenter )
         :setMinimumSize( QSize( 100, 100 ) )
         :setDisplayedWhenStopped( .F. )
...
//STATIC PROCEDURE UDFstart(oForm)
//   LOCAL oBusy

//   oBusy := hqlProgressIndicator(/*name*/, oForm)
//   oBusy:setMinimumSize( QSize( 35, 35 ) )
//   oBusy:hqlAddMeToLayout( oForm:centralWidget:layout() )
//   oBusy:hqlAlignMeToLayout( oForm:centralWidget:layout(), Qt_AlignCenter )
//   oBusy:show()
//   oBusy:startAnimation()
