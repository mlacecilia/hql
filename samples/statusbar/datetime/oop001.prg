...
STATIC PROCEDURE UDFshowMainWindow()
   ...
   WITH OBJECT oWnd := hqlMainWindow(/*name*/)
      :setWindowTitle( "HQLSTATUSBAR tester" )
      ...
      WITH OBJECT hqlMenuBar(/*name*/)
         ...
         WITH OBJECT :hqlAddMenu(/*name*/)
            :hqlCaption( "&Tools" )
            WITH OBJECT :hqlAddAction( /*name*/ )
               :setText( "show status message" )
               :hqlOnTriggered( { || oWnd:mystatus:showMessage( "this is the message", 4000 ) } )
            END WITH
         END WITH

      END WITH

      WITH OBJECT hqlStatusBar( "mystatus" )
         :setFont( oFont )
         WITH OBJECT :hqlInsertPermanentWidget( -1, hqlLabel( /*name*/ ), /*nStretch*/ )
            :setText( "permanent widget" )
         END WITH

         WITH OBJECT :hqlInsertWidget( -1, hqlLabel( /*name*/ ), /*nStretch*/ )
            :setText( "widget" )
         END WITH

         WITH OBJECT :hqlStatusDateTime( /*name*/ )
            //:hqlSetFormat( QLocale_ShortFormat | QLocale_LongFormat | QLocale_NarrowFormat )
            //:hqlSetType( HQL_STATUSDTTIME | HQL_STATUSDTDATE | HQL_STATUSDTALL )
         END WITH
      END WITH
