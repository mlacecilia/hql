...
*/
CLASS hql_atModel INHERIT hb_hqlAbstractTableModel, hql_qobj
...
RETURN QVariant() //NIL  // always NIL if not managed, in this way C++ returns QVariant()
...
METHOD __queryData( nInfoType, nRole, nA, nB, value ) CLASS hql_atModel
   ...
   // experimental CASE HBQT_QAIM_canFetchMore
   // experimental    RETURN ::__canFetchMore( nRole, nA, nB)

   // experimental CASE HBQT_QAIM_fetchMore
   // experimental    RETURN ::__fetchMore( nRole, nA, nB)

   ENDSWITCH

RETURN NIL  // always NIL if not managed, in this way C++ returns QVariant()
