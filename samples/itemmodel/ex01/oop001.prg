...
STATIC PROCEDURE UDshowInfo( oTview )
   ...
   oIndex := oTview:model:index( 3, 3 ) //   it returns invalid index
   hql_Trace( "isValid=" + hb_ValToExp( oIndex:isValid() ) + ;
                      " row=" + hb_Ntos( oIndex:row() ) + ;
                      " column=" + hb_Ntos( oIndex:column() ) )

   oIndex := oTview:model:hbCreateIndex( 3, 3 ) // helper to create a valid index
   hql_Trace( "isValid=" + hb_ValToExp( oIndex:isValid() ) + ;
                      " row=" + hb_Ntos( oIndex:row() ) + ;
                      " column=" + hb_Ntos( oIndex:column() ) )

   nFlags := oTview:model:hbFlags( oTview:model:index( 3, 3 ) )
   hql_Trace( "nFlags=" + hb_Ntos( nFlags ) )

   nFlags := oTview:model:hbFlags( oTview:model:hbCreateIndex( 3, 3 ) )
   hql_Trace( "nFlags=" + hb_Ntos( nFlags ) )
